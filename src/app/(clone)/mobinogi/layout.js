...
export default function MobinogiLayout ({ children }) {
  ...
  return (
    <>
      ...
      <DndProvider backend={ HTML5Backend }>
              ...
              <div className={ classNames("flex", "flex-row", "flex-wrap") }>
                  {
                    ...
                    <div className={ classNames("flex", "flex-column", "col-1") }>
                          <DraggableCard
                            key={ rune.runeId }
                            id={ rune.runeId }
                            src="/images/rune.png"
        ...
        <Dialog visible={ showDialog } header={`${selectedRune?.runeSlot}룬: ${selectedRune?.runeNm}`} onHide={() => setShowDialog(false) } modal>
          <p>{ selectedRune?.runeDesc }</p>
        </Dialog>
      </DndProvider>
    </>
  );
